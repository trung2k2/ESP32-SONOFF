...
void wifi_data_callback(char *data, int len)
{
    ...
    //ESP_LOGI(TAG, "SSID:%s - wifi_data_callback", ssid);
    //ESP_LOGI(TAG, "PASSWORD:%s - wifi_data_callback", password);
}
void app_config(void) 
{
    ...
    {
        ESP_LOGI(TAG, "chua co ket noi");
        if(provisition_type == PROVISION_SMARTCONFIG)
        {
            ESP_LOGI(TAG, "dung smart config ");
            ESP_ERROR_CHECK(esp_wifi_start() );
        ...
        {
            ESP_LOGI(TAG, "dung access point ");
            ap_start();
            start_webserver();
